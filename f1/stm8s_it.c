...
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
//extern uint8_t HEADER_ADDRESS_Write = ((SLAVE_ADDRESS  >> 7 ) | 0xF0);
//extern uint8_t HEADER_ADDRESS_Write = ((SLAVE_ADDRESS  << 1) | 0x00);
//extern uint8_t HEADER_ADDRESS_Write = (SLAVE_ADDRESS | 0x00);

extern uint8_t HEADER_ADDRESS_Write = (((SLAVE_ADDRESS & 0xFF00) >> 7) | 0xF0);
extern uint8_t HEADER_ADDRESS_Read;
/* Private variables ---------------------------------------------------------*/
//__IO uint8_t TxBuffer[BUFFERSIZE] = {0X7c, 0x00, 0xfd, 0x91};
__IO uint8_t TxBuffer[BUFFERSIZE] = 
															{/*Sw_Rst*/0X00, 0XFD, 0X91,
																/*Ini_Seque*/ 0x00 ,0xC0, 0xFD, 0X90, 0XA0, 0XB0, 0XC0, 0XE0, 0XFC, 0XEE, 0XF0, 																0XF8, 0XE0, 0X00,     						
																/*Send_Data */ 0X00, 0X44,
																/*Display_ON_Seuqe */ 0x00 , 0xFD, 0X90, 0XA0, 0XB0, 0XC0, 0XE0, 0XFC, 0XEE, 																0XF0, 0XF8, 0XC8
															};
//	__IO uint8_t TxBuffer[BUFFERSIZE];														
...
INTERRUPT_HANDLER(I2C_IRQHandler, 19)
{
  ...
  switch (I2C_GetLastEvent())
  {
    ...
    case I2C_EVENT_MASTER_MODE_ADDRESS10:
      ...
      /* Send slave Address for write */
      I2C_Send7bitAddress(SLAVE_ADDRESS, I2C_DIRECTION_TX);
			GPIO_WriteLow(GPIOD, GPIO_PIN_0);  // LED set means writeLOW
	    Delay(0xFFFF);
    ...
    case I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED:
	  	GPIO_WriteReverse(GPIOD, GPIO_PIN_0);  // LED set means writeLOW
	    Delay(0xFFFF);
			GPIO_WriteReverse(GPIOD, GPIO_PIN_0);  // LED set means writeLOW
	    Delay(0xFFFF);
			GPIO_WriteReverse(GPIOD, GPIO_PIN_0);  // LED set means writeLOW
  ...
  switch (I2C_GetLastEvent())
  {
    ...
    case I2C_EVENT_MASTER_MODE_ADDRESS10:
      ...
      I2C_Send7bitAddress(SLAVE_ADDRESS, I2C_DIRECTION_TX);
				...
				GPIO_WriteLow(GPIOD, GPIO_PIN_0);  // LED reset means writeHigh
				Delay(0xFFFF);
				Delay(0xFFFF);
				Delay(0xFFFF);
				Delay(0xFFFF);
